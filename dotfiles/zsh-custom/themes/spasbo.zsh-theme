prompt_spasbo_precmd(){
  if git rev-parse --git-dir > /dev/null 2>&1; then
    ...
    _STATUS="$_STATUS|"
    # Unstaged
    if $(echo "$_GIT_DIR_STATUS" | grep '^[[:space:]][MD] ' &> /dev/null); then
      _STATUS=$_STATUS$ZSH_THEME_GIT_PROMPT_UNSTAGED
    fi
    # Staged
    if $(echo "$_GIT_DIR_STATUS" | grep '^[AM]. ' &> /dev/null); then
      _STATUS=$_STATUS$ZSH_THEME_GIT_PROMPT_STAGED
    fi
    # Deleted
    if $(echo "$_GIT_DIR_STATUS" | grep "^[[:space:]]\?D" &> /dev/null); then
      _STATUS=$_STATUS$ZSH_THEME_GIT_PROMPT_DELETED
    fi
    # Moved
    if $(echo "$_GIT_DIR_STATUS" | grep "^[[:space:]]\?R" &> /dev/null); then
