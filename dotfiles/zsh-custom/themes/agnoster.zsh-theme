...
prompt_context() {
  ...
  if [[ "$user" != "$DEFAULT_USER" || -n "$SSH_CONNECTION" ]]; then
    ...
    #prompt_segment $PRIMARY_FG default '%{%b%f%}%{%F{blue}%} %n%{%F{magenta}%}@%{%F{cyan}%}%m %{%F{black}%}'
  fi
}
...
prompt_status() {
  local symbols
  symbols=()
  symbols+="%(?:%{$fg_bold[green]%}$PROMPT_EXIT_CODE_OK:%{$fg_bold[red]%}$PROMPT_EXIT_CODE_ERROR)%{%b"
