...
#+PROPERTY: header-args :tangle /home/steven/.emacs.d/emacs-init.el :comments org
...
(defconst *is-window* (not (fboundp window-system)))
...
(use-package ispell
  ...
  :config
  (let ((hun "hunspell"))
    (if (executable-find hun)
        (setq-default ispell-program-name hun)))
...
;;   :ensure
;;   :init
;;   (exec-path-from-shell-getenv "GOBIN")
;;   :pin melpa
;;   :mode "\\.go\\'"
;;   :config
;;   (go-eldoc-setup)
