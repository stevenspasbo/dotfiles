...
(defconst *is-window* (display-graphic-p))
...
(use-package helm
  ...
  :init
  (use-package helm-mode :diminish helm-mode)
...
*** helm-swoop
#+BEGIN_SRC emacs-lisp
;; Package was removed in https://github.com/melpa/melpa/pull/9520.
;; Replacement options: https://www.reddit.com/r/emacs/comments/1pnra4c/helmswoop_replacement
...
(use-package company
  ...
  (setq company-minimum-prefix-length 2
        ...
        company-global-modes '(not erc-mode eshell-mode sh-mode)
...
;; Optional: Helper to install all of them at once
(defun my-install-go-grammars ()
  "Install Go and Gomod grammars if they are missing."
  (interactive)
...
(use-package go-mode
  :ensure t
  :defer t
  :mode "\\.go\\'" ;; Associate .go files
  :config
  ;; Helper to run gofmt/goimports on save
  ...
  ;; Hooks for modern Go setups
  (add-hook 'go-ts-mode-hook (lambda () (flycheck-mode -1)))
  (add-hook 'go-ts-mode-hook 'eglot-ensure) ; Start LSP automatically
  (add-hook 'before-save-hook 'my-go-format-buffer)) ; Format on save

;; Note: You might need to remap go-mode to go-ts-mode if you prefer the TS version default
